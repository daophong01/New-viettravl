...
export default function UserWatchLaterPage() {
  ...
  const load = async () => {
    ...
    const res = await fetch(`${base}/api/watchlater/tours`, {
      headers: { Authorization: `Bearer ${token || ""}` },
    });
    if (res.ok) setTours(await res.json());
  };

  const removeWatch = async (tourId: number) => {
    const base = process.env.NEXT_PUBLIC_API_BASE_URL || "";
    const res = await fetch(`${base}/api/watchlater/${tourId}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token || ""}` },
    });
    if (res.ok) {
      window.dispatchEvent(new CustomEvent("watch-changed", { detail: { delta: -1 } }));
      push({ text: "Đã bỏ khỏi xem sau", type: "success" });
      await load();
    } else {
  ...
  return (
    <div className="py-8 space-y-6">
      <h1 className="text-2xl font-bold">Xem sau của tôi</h1>
      {tours.length === 0 ? (
        <p className="text-sm">Chưa có tour xem sau. Hãy thêm từ trang danh sách hoặc chi tiết tour.</p>
      ) : (
